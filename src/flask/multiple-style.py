...
class ContentLoss(nn.Module):
    ...
    def forward(self, input):
        self.loss = F.mse_loss(input, self.target[:, :, :input.size(2), :input.size(3)])
        return input
...
def get_style_model_and_losses(cnn, normalization_mean, normalization_std, style_imgs, content_img, content_layers=None, style_layers=None):
    ...
    for layer in cnn.children():
        if isinstance(layer, nn.Conv2d):
            ...
            raise RuntimeError(f'Unrecognized layer: {layer.__class__.__name}')
        ...
        if name in style_layers:
            # Process each style image and accumulate the style losses
...
def run_style_transfer(cnn, normalization_mean, normalization_std,
                       ...
                       style_layers=None, optimizer=None):
...
def upload_images():
    if 'content' not in request.form or 'style1' not in request.form or 'style2' not in request.form:
        return jsonify({'error': 'Content, style1, and style2 images are required.'}), 400

    try:
        content_image = decode_base64_image(request.form['content'])
        style_image1 = decode_base64_image(request.form['style1'])
        style_image2 = decode_base64_image(request.form['style2'])
    except ValueError as e:
        return jsonify({'error': str(e)}), 400

    # Convert PIL Images to tensors
    content_img = image_loader(content_image)
    style_img1 = image_loader(style_image1)
    style_img2 = image_loader(style_image2)
    ...
    output = run_style_transfer(cnn, cnn_normalization_mean, cnn_normalization_std,
                            content_img, [style_img1, style_img2], input_img, content_layers=content_layers_default,
